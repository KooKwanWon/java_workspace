...
public class B_Casting {
	...
	public void autoCasting() {
		...
		float f = l; // 4byte(표현 가능한 수의 범위가 더 크기 떄문에 자동 형 변환이 가능)
		double d = f;// 8byte 
		...
		/* 강제 형 변환 (명시적 형 변환)
			...
			- 범위가 큰 크기의 자료형의 데이터를 작은 크기의 
			  자료형으로 변환하려고 할 떄 사용한다.
			- 강제 형 변환의 경우 데이터 손실이 발생할 수 있다.  
		*/
		public void casting() {
			double d = 4.62345678904567890; //15wkfltn
			...
			int number2 = 128;
			byte bNumber2 =(byte)number2; // -128 ~ 127
			...
			*				  쌍방향으로 형변환 가능 (유니코드)
			*
			*  char은 음수값 저장 불가 -> 값의 범위가 0~65535
			*/
			int num = 'A';
